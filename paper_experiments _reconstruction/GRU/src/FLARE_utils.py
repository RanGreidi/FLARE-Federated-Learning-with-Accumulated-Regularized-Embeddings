...
def client_update_fn(tf_dataset, server_weights, accumolator, prun_percent, learning_rate, E, tau,u):
  ...
  pruned_client_weights, accumolator = client_update(models_0, tf_dataset, server_weights, accumolator, client_optimizer, prun_percent, E) 
...
def evaluate(server_state,central_emnist_test):
  ...
  res = keras_model.evaluate( central_emnist_test, steps=2, verbose=0)
  return res
# def evaluate(server_state,central_emnist_test):
#   keras_model = create_keras_model()
#   keras_model.compile(
