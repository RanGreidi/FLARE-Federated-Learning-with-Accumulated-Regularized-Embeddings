...
def log_writer(experiment_name,
                ...
                E_logger_FFL,
                output_list,example_list, a,b,c,d,e,round):
    ...
    with open(os.path.join('results/' + experiment_name, 'FedProx_evaluation'), "w") as Second_FedAlgo_file:
    ...
    #write furth Algo loss to text file
    with open(os.path.join('results/' + experiment_name, 'EF_topk'), "w") as fourth_FedAlgo_file:
        toFile = "\n".join(str(item) for item in history_fourth_algo_server_state)
        fourth_FedAlgo_file.write(toFile) 
    ...
    output_list.append(str(
                        ...
                        'FedProx_evaluation:   '     + str(c[1]) + '\n'
                        'EF21_evaluation:   '     + str(d[1]) + '\n'
                        'EF_And_Sparsification:   '     + str(e[1]) + '\n' +'\n'
                        ))
    ...
    #write output list for linux terminal       
    with open(os.path.join('results/' + experiment_name, 'examples'), "w") as outputfile:
        toFile = "\n".join(str(item) for item in example_list)
        outputfile.write(toFile)    
    #print('\n')